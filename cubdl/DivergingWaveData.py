# File:       DivergingWaveData.py
# Author:     Dongwoon Hyun (dongwoon.hyun@stanford.edu)
# Created on: 2020-05-26
import numpy as np


class DivergingWaveData:
    """ A template class that contains diverging wave data.

    DivergingWaveData is a container or dataclass that holds all of the information
    describing a focused transmit acquisition. Users should create a subclass that
    reimplements __init__() according to how their data is stored.

    The required information is:
    idata       In-phase (real) data with shape (nxmits, nchans, nsamps)
    qdata       Quadrature (imag) data with shape (nxmits, nchans, nsamps)
    tx_ori      List of virtual transmit source origins with shape (N,3) [m]
