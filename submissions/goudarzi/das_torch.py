...
class DAS_PW(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        ch1 = torch.zeros([npixels, 2, nelems], dtype=self.dtype, device=self.device)
        apod1 = torch.zeros([npixels, nelems], dtype=self.dtype, device=self.device)
        ...
        for t, td, ta in tqdm(zip(self.ang_list, txdel, txapo), total=nangles):
            for r, rd, ra in zip(self.ele_list, rxdel, rxapo):
                ...
                ## additional part
                ch1[0:npixels, 0, r] = ifoc * apods
                ch1[0:npixels, 1, r] = qfoc * apods
                apod1[0:npixels, r] = apods
...
class DAS_FT(torch.nn.Module):
    ...
    def __init__(
        self, F, grid, rxfnum=1.75, dtype=torch.float, device=torch.device("cuda:0")
    ):
...
def apod_focus(grid, ele_pos, fnum=1.75, min_width=1e-3):
